...
tm = benchmark.TestManager(options=flags, working_prefix=arguments['--resultsdir'])
...
print "All on", all_1_val, all_1_val_time
...
all_0_val = r[0]
all_0_val_time = r[1]
print "All off", all_0_val, all_0_val_time
...
if arguments["--choose-matrix"] is None:
    ...
    m2.loadMatrix("51factors 4096runs resolution5")
else:
    m = fracfact.FactorialMatrix(len(flags))
...
fp = open("main_effects_llvm/{}/{}_O1".format(arguments['PLATFORM'],arguments['BENCHMARK']),"w")

for i, f in enumerate(flags):
    r = (m.getFactor(i), str(i)+" "+f, m2.getFactor(i), plot_sig[i])
    fp.write("{} {}\n".format(r[0], r[2]))
    print r[0], r[2]
    results.append(r)
...
significant_hi_x_text = min([significant_hi_x_text, 49.0])
...
# Titles
figtext(.5,.96,"Individual flag's effect on energy consumption in LLVM (percentage relative to no optimisations)", fontsize=15, ha='center')
figtext(.5,.93,'{0}, {1}, Flags enabled by O1, fractional factorial design of 4096 runs'.format(arguments['PLATFORM'],arguments['BENCHMARK']), fontsize=12, ha='center')
