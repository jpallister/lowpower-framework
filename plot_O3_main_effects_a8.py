...
m_1.loadMatrix("9factors 256runs")
m_2.loadMatrix("9factors 256runs")
...
for i, f in enumerate(flags):
    results.append((m.getFactor(i), m_1.getFactor(i), m_2.getFactor(i), f, m2.getFactor(i), plot_sig[i]))
...
# plot energies
rects1 = ax1.bar(range(len(rx)), ry, align='center', width=0.2, color='#006600')
rects1_1 = ax1.bar(map(lambda x: x+0.2, range(len(rx))), ry_1, align='center', width=0.2, color='y')
rects1_2 = ax1.bar(map(lambda x: x+0.4, range(len(rx))), ry_2, align='center', width=0.2, color='r')
# plot times
rects2 = ax1.bar(map(lambda x: x + 0.6, range(len(rx))), ryt, align='center', width=0.2, color='c', hatch="/")
