...
if arguments['--resultsdir'] is None:
    arguments['--resultsdir'] = "testing_all_f1/{0}/{1}".format(arguments['PLATFORM'], arguments['BENCHMARK'])
...
if arguments["--choose-matrix"] is None:
    ...
    m_1.loadMatrix("37factors 512runs resolution4")
    m_2.loadMatrix("37factors 512runs resolution4")
    m2.loadMatrix("37factors 512runs resolution4")
else:
    m = fracfact.FactorialMatrix(len(flags))
...
fp = open("main_effects_f1/{}/{}_O1".format(arguments['PLATFORM'],arguments['BENCHMARK']),"w")

for i, f in enumerate(flags):
    r = (m.getFactor(i), m_1.getFactor(i), m_2.getFactor(i), f, m2.getFactor(i))
    fp.write("{} {}\n".format(r[0], r[2]))
    results.append(r)
...
for v1, v3, v4, flag, v2 in results:
    f.write(flag+"\n")
f.close()
